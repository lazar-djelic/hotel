...
import { LoaderIcon, AlertCircleIcon, ArrowLeftIcon } from "lucide-react";
  ...
  if (stay && sessionData.type === "stay") {
    ...
    return (
      ...
      >
        ...
        <CancelTotalSection grandTotal={grandTotal} curr={curr} t={t} />
...
const CancelIndicator = () => {
  ...
  return (
    <div className="bg-warning/10 border border-warning/50 rounded-xl p-4 mb-6 flex items-center gap-3">
      <AlertCircleIcon className="size-8 text-warning flex-shrink-0" />
      <div>
        <p className="font-semibold text-warning">{t("payment.paymentCanc")}</p>
      </div>
    </div>
  );
};
      ...
      <button
        ...
        onClick={() =>
          navigate(ROUTES.PAYMENT.CHECKOUT, {
            state: {
              stay: stayId || undefined,
              roomres: roomresId || undefined,
              amres: amresId || undefined,
            },
          })
        }
      >
