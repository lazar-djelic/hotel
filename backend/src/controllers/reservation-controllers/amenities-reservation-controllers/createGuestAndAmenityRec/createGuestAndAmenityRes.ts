...
import {
  CreateGuestAndAmenityResBodySchema,
  type CreateGuestAndAmenityResRequest,
} from "./types.ts";

export const createGuestAndAmenityRes = async (
  req: CreateGuestAndAmenityResRequest,
  res: Response,
  next: NextFunction,
) => {
  try {
    console.log("-------------------");
    const parsed = CreateGuestAndAmenityResBodySchema.safeParse(req.body);
    ...
    try {
      ...
      const parsedGuest = guestSchema.parse(createdGuests[0]);
